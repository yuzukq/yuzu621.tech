...
Rubyをやって思ったのは考えを表現しやすい言語だなと思った．
...
ここまで抽象化された言語はすごいと感じ，内部実装も少しだけ味見してみました．Ruby公式の処理系MRI Ruby(Matz' Ruby Implementation)はRubyで記述されたソースコードはCで実装されています[hoge]([https://www.ruby.or.jp/ja/tech/install/ruby/implementations](https://www.ruby.or.jp/ja/tech/install/ruby/implementations))．例えば組み込み関数の中でも簡単なsumメソッドの内部実装を見てみると，
...
コロン：で始まる軽量な識別子．イミュータブルなオブジェクトとして宣言でき，文字列型での宣言と違い，共通のオブジェクトとして付与されるためメモリ効率が良い(比較的)
...
呼び出しは`person[:name]`のように[]で書き，`person.name`のような呼び出しがRailsプロジェクトでみられるのはアクティブヘルパーによるもの

アクティブヘルパーがどのようにして，Ruby標準のクラスメソッドに便利な関数を提供しているかは「Rubyのオブジェクト指向/モンキーパッチ」の節で言及している．
...
>> 配列を引数として受け取り，合計・平均・最大・最小を返す関数の実装．
...
やはり型があるやりやすさと低レイヤでのパフォーマンスを考えると落ち着く部分もあるが，可読性の高さとWeb領域での親和性の高さのあるRubyを触っていると適材適所だなと感じてます．

低レイヤの実装を深くまで気にすることなく，ここまで抽象化されて書き心地の良い言語での開発体験を築き上げてくれた歴戦のエンジニアの方々には本当に頭が上がりません．

## ハッシュを使ったメモ化

Rubyらしいかと言えば他の言語でも利用される手法ですが，再帰処理において一度やった計算結果をキャッシュして置くメモ化においてもハッシュが利用できます．
...
変数や関数名をスネークケースで書くのだけでなく，その関数の特性に合わせた命名規則の違いもある．Ruby標準の組み込みでは，!(bang)-破壊メソッドや，?(question)-真偽値を返すメソッドでこれらの記号を末尾に着けるといった慣習になっている．
...
動的型付け言語であるRubyならではの柔軟性．オブジェクトを判断するときに，*型よりも「メソッドを持っているか」が重要を重要視する．*
...
Rubyでは前述のとおり，すべてがオブジェクトである[リファレンス]()という特徴を持っている．このため，Pythonなどの他の言語と違い，Integerクラスや，Arrayクラスが持つ標準メソッドのほかに独自のメソッドをオーバーライドすることが出来る．[参考]([https://qiita.com/YutoYasunaga/items/7dc5c418fc90f84f274e](https://qiita.com/YutoYasunaga/items/7dc5c418fc90f84f274e))
...
モンキーパッチは実際にRailsのアクティブレコードで利用されている．例えばNumericクラスに対しては日付の拡張として使われている．
...
# > Ruby World Hello
# split ↔ join の関係(文字列かは配列 ↔ 配列から文字列)
```
